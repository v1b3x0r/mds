...
/**
 * Cognitive link between two entities
 */
export interface CognitiveLink {
  target: string          // Entity ID
  strength: number        // Connection strength (0..1)
  formed: number          // Timestamp when link was created
  lastReinforced: number  // Last interaction time
  bidirectional: boolean  // If true, target also links back
}

/**
 * Options for creating cognitive link
 */
export interface CognitiveLinkOptions {
  strength?: number
...
export class CognitiveLinkManager {
   ...
   */
  static connect(
    links: Map<string, CognitiveLink>,
    targetId: string,
    timestamp: number,
    options: CognitiveLinkOptions = {}
  ): CognitiveLink {
   ...
   */
  static disconnect(links: Map<string, CognitiveLink>, targetId: string): boolean {
   ...
   */
  static getStrength(links: Map<string, CognitiveLink>, targetId: string): number {
   ...
   * Check if connected
   */
  static isConnected(links: Map<string, CognitiveLink>, targetId: string): boolean {
   ...
   */
  static getConnected(links: Map<string, CognitiveLink>): string[] {
   ...
   */
  static getSortedByStrength(links: Map<string, CognitiveLink>): CognitiveLink[] {
   ...
   */
  static count(links: Map<string, CognitiveLink>): number {
    return links.size
  }
   ...
   */
  static averageStrength(links: Map<string, CognitiveLink>): number {
