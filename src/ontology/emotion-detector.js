 ...
 */
export const THAI_EMOTION_KEYWORDS = {
 ...
 */
export const ENGLISH_EMOTION_KEYWORDS = {
    'shy': 'อาย',
    'ashamed': 'อาย',
    'embarrassed': 'อาย',
    'lonely': 'เหงา',
...
export function detectEmotionFromText(text) {
    ...
    for (const [keyword, emotionLabel] of Object.entries(THAI_EMOTION_KEYWORDS)) {
        if (normalized.includes(keyword.toLowerCase())) {
            ...
            const baselineEmotion = EMOTION_BASELINES[emotionLabel];
            if (baselineEmotion) {
 ...
 */
export function detectAllEmotions(text) {
    if (!text)
        return [];
    const normalized = text.toLowerCase();
    const detected = [];
 ...
 */
export function blendMultipleEmotions(emotionLabels) {
    ...
    for (const label of emotionLabels) {
        if (EMOTION_BASELINES_TH[label]) {
            ...
            const baselineEmotion = EMOTION_BASELINES[label];
            if (baselineEmotion) {
 ...
 */
export function findClosestThaiEmotion(state) {
    let closest = 'ปกติ';
