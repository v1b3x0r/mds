...
export class SemanticSimilarity {
     ...
     */
    async similarity(entityA, entityB) {
     ...
     */
    async similarityMatrix(entities) {
     ...
     */
    async findTopSimilar(target, candidates, n = 5) {
        const similarities = [];
        for (const candidate of candidates) {
            if (candidate.id === target.id)
                continue;
 ...
 */
export function createOpenAISemantic(apiKey, model) {
 ...
 */
export function createMockSemantic() {
