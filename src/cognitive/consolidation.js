    ...
    consolidate(memories) {
        ...
        for (const [_key, groupMemories] of groups) {
            if (groupMemories.length < 2)
                continue; // Need at least 2 to consolidate
            // Sort by timestamp
            groupMemories.sort((a, b) => a.timestamp - b.timestamp);
            // Check if already consolidated
            const existing = Array.from(this.consolidated.values()).find(cm => cm.type === groupMemories[0].type && cm.subject === groupMemories[0].subject);
            if (existing) {
 ...
 */
export function memorySimilarity(memA, memB) {
