...
export class LearningSystem {
    experiences = [];
    patterns = new Map();
    actionValues = new Map(); // Q-values for actions
    config;
    constructor(config = {}) {
        this.config = {
    ...
    detectPatterns() {
        ...
        if (actions.length < this.config.patternMinLength)
            return;
     ...
     */
    getBestAction(availableActions) {
     ...
     */
    forgetOldPatterns(currentTime, maxAge = 300000) {
 ...
 */
export function createExperience(action, outcome, reward, context = {}) {
    return {
        timestamp: Date.now(),
        action,
        context,
 ...
 */
export function calculateReward(outcome, expectedOutcome) {
    // Exact match
